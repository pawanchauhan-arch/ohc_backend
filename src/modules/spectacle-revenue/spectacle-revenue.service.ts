...
@Injectable()
export class SpectacleRevenueService {
  constructor(
  ...
  async create(
    ...
    return this.spectacleRevenueModel.create({
...
if (from && to) {
  ...
  where.RevenueDate = {
    ...
    const { rows, count } =
      await this.spectacleRevenueModel.findAndCountAll({
        where,
  ...
  async getById(
    ...
    const data =
      await this.spectacleRevenueModel.findByPk(
        id,
      );
  ...
  const rows =
    await this.spectacleRevenueModel.findAll({
      where,
      order: [['RevenueID', 'DESC']],
    });
...
const worksheet = workbook.addWorksheet('Spectacle Revenue');
